	   	...
	   	try {	   		
	    	...
	    	endButton.setOnAction(new EventHandler<ActionEvent>() {
	    	    ...
	    	    public void handle(ActionEvent event) {
	    	    	...
	    	    	else if(endButton.getText().equals("종 료 하 기")){
		    	    	send("quit" + userName);
		    	    	try{
		    	    		Thread.sleep(100);
		    	    	}catch(Exception e){
				...
				public void run() {
					...
					try {
						if(count == 5) {
							count = 0;
							timerField.setText("");
							userResultLog = "정답 : " + correctResult + ", 오답 : " + incorrectResult;
							System.out.println("정답 : " + correctResult + ", 오답 : " + incorrectResult);
							try {sleep(50);} catch(Exception e) {} Timer.interrupted();
							send("endquiz");
							chatlog.appendText(userName.substring(5) + " > " + "정답 : " + correctResult + ", 오답 : " + incorrectResult + "\n");
							correctResult = 0;
							incorrectResult = 0;
			...
			try {
				...
				Platform.runLater(()->{
					if(message.length() >= 1 && message.substring(0,1).equals("m")) {
						...
						else if(message.length() >= 4 && message.substring(0, 4).equals("m다음/")) {}
		...
		Thread thread = new Thread() {
			public void run() {
				try {
					...
					if(temp.length() >= 2 && temp.substring(0,2).equals("답:")) {
						System.out.println(temp);
